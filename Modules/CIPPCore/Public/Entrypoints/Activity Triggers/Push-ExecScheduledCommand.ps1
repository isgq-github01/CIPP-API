function Push-ExecScheduledCommand {
    ...
    if ($Results) {
        ...
        $HTMLNoStyle = $FinalResults -replace '<table>', "This alert is for tenant $Tenant. <br /><br />" | Out-String
        ...
        switch -wildcard ($task.PostExecution) {
            '*psa*' { Send-CIPPAlert -Type 'psa' -Title $title -HTMLContent $HTMLNoStyle -TenantFilter $Tenant }
